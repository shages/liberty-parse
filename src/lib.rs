...
pub fn parse(input: &str) -> Result<Vec<GroupItem>, VerboseError<&str>> {
    match parse_libs::<VerboseError<&str>>(input) {
        Ok((_, libs)) => Ok(libs),
        Err(Err::Error(err)) | Err(Err::Failure(err)) => Err(err),
        _ => Ok(vec![]),
    }
}
...
"#
    ...
    macro_rules! parse_file {
        ($fname:ident) => {{
            ...
            let (_, _result) = parse_libs::<(&str, ErrorKind)>(data).unwrap();
        }};
    }
